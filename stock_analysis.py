...
def get_market_data():
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 打印前5个股票代码用于调试
            if '代码' in df.columns:
                print(f"前5个股票代码: {df['代码'].head().tolist()}")
...
def get_stock_data_from_market(stock_code, market_df):
    ...
    if market_df is None or market_df.empty:
        print(f"全市场数据为空，无法获取股票 {stock_code} 数据")
        return None
    ...
    try:
        ...
        # 查找股票 - 多种匹配方式
        # 方法1: 精确匹配
        stock_data = market_df[market_df[code_col] == stock_code]
        
        if stock_data.empty:
            # 方法2: 去除可能的字母前缀后缀后匹配
            cleaned_codes = market_df[code_col].astype(str).str.replace(r'[a-zA-Z.]', '', regex=True)
            stock_data = market_df[cleaned_codes == stock_code]
        
        if stock_data.empty:
            # 方法3: 只匹配后6位
            stock_data = market_df[market_df[code_col].astype(str).str[-6:] == stock_code]
...
def generate_stock_reports():
    ...
    for stock_info in YOUR_STOCKS:
        ...
        if not stock_data:
            fail_count += 1
            print(f"❌ 股票 {stock_name}({stock_code}) 数据获取失败，跳过")
            # 创建错误卡片
            error_html = f"""
...
"""
    ...
    print(f"\n✅ 分析完成: 成功 {success_count} 支，失败 {fail_count} 支")
    
    if not reports:
        error_html = f"""
    <div class="stock-card error-card">
        <div class="stock-header">
            <div class="stock-name">❌ 所有股票数据获取失败</div>
        </div>
        <div class="ai-analysis">
            <strong>⚠️ 系统异常:</strong><br>
            当前无法获取任何股票的实时数据，请检查网络连接、数据源或API密钥配置。
        </div>
    </div>
"""
        reports.append(error_html)
